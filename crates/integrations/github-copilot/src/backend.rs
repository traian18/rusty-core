    ...
    async fn execute(
        ...
        let mut child = command
            ...
            .map_err(|error| ExecutionError::BackendError {
                message: format!("failed to spawn Copilot CLI: {error}"),
        ...
        let status = child
            ...
            .await
            .map_err(|error| ExecutionError::BackendError {
                message: format!("Copilot CLI process error: {error}"),
                code: "wait_failed".into(),
            })?;
        let text = parse_output(&output).map_err(|message| ExecutionError::BackendError {
            message: if error_output.is_empty() {
                format!("{message} (exit status: {status})")
            } else {
                "Copilot CLI reported an error; run `copilot login` and retry".into()
            },
...
impl IntegrationFactory for GitHubCopilotFactory {
    ...
    fn descriptor(&self) -> BackendDescriptor {
        GitHubCopilotBackend::new(GitHubCopilotConfig::default()).descriptor()
    }
    async fn create(
        &self,
        config: serde_json::Value,
    ) -> Result<Arc<dyn ExecutionBackend>, Box<dyn std::error::Error + Send + Sync>> {
        Ok(Arc::new(GitHubCopilotBackend::new(serde_json::from_value(
            config,
        )?)))
    }
}
