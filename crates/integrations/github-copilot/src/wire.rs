...
pub fn safe_error(value: &serde_json::Value) -> Option<String> {
    let kind = value
        ...
        .unwrap_or_default();
    if !kind.contains("error") && value.get("error").is_none() {
        return None;
    }
    value
        .get("error")
...
pub fn parse_output(bytes: &[u8]) -> Result<String, String> {
    ...
    for value in values {
        if let Some(error) = safe_error(&value) {
            return Err(error);
        }
        if let Some(delta) = assistant_text(&value) {
            text.push_str(&delta);
        }
    }
    if text.is_empty() {
        Err("Copilot CLI response did not contain assistant text".into())
    } else {
        Ok(text)
    }
}
...
mod tests {
    ...
    fn parses_supported_fixture_shapes() {
        assert_eq!(
            assistant_text(&serde_json::json!({"content":"hello"})).as_deref(),
            Some("hello")
        );
        assert_eq!(
            assistant_text(&serde_json::json!({"data":{"message":"nested"}})).as_deref(),
            Some("nested")
        );
    }
