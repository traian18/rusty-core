//! Tests for session lifecycle, drain, restore, and contract polymorphism.
...
use harness_protocol::ids::{BackendId, RequestId, SessionId};
...
use crate::integration::{IntegrationFactory, IntegrationRegistry};
use crate::scheduler::{Scheduler, SchedulerConfig};
use crate::session_manager::contracts::{
    SessionLifecycle, SessionRegistry, SessionRestorer, SessionService,
};
...
use crate::traits::{EventSink, ExecutionBackend};
...
/// consumed by `SessionSupervisor::supervise` at session-creation time. So
...
#[tokio::test]
async fn session_manager_implements_traits_and_contract_polymorphism() {
    let manager = Arc::new(SessionManager::new(Arc::new(Scheduler::new(
        SchedulerConfig::default(),
    ))));

    // Verify polymorphic usage as dyn SessionRegistry
    let registry: Arc<dyn SessionRegistry> = manager.clone();
    assert_eq!(registry.active_session_count().await, 0);
    assert!(registry.active_session_ids().await.is_empty());

    // Verify polymorphic usage as dyn SessionLifecycle
    let lifecycle: Arc<dyn SessionLifecycle> = manager.clone();
    ...
    let _restorer: Arc<dyn SessionRestorer> = manager.clone();

    // Verify polymorphic usage as dyn SessionService
    let service: Arc<dyn SessionService> = manager.clone();
    assert_eq!(service.active_session_count().await, 1);

    lifecycle
        .close_session(runtime.session_id)
        .await
        .expect("close_session through SessionLifecycle contract");
    assert_eq!(registry.active_session_count().await, 0);
}

#[tokio::test]
async fn active_session_registry_operations() {
    ...
    registry.register(session_id, runtime.clone(), permit).await;
    assert_eq!(registry.count().await, 1);
    assert!(registry.contains(session_id).await);
    assert_eq!(registry.ids().await, vec![session_id]);
